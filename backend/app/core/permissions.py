...
class AuthContext:
    ...
    *is* the asker. Like `channel_identity_id` it decides no permission - it
    exists so `per_user` memory can refuse to attribute a stranger's note to the
    ...
    stand-in constructor must set it too, or a `per_user` run silently reads the
    owner's store as if it were the visitor's. `test_publisher_standing.py` pins
