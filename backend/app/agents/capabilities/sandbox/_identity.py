...
SessionScope = Literal["run", "conversation", "user", "agent"]
BackendKind = Literal["state", "docker", "daytona"]
...
def scope_key(identity: WorkspaceIdentity, scope: SessionScope, backend: BackendKind) -> str:
    ...
    The backend kind is folded in so that changing `state` to `docker` on a live
    conversation opens a new workspace instead of reattaching to one of a
    ...
    key = f"{backend[0]}{scope[0]}-{identity.organization_id.hex[:8]}-{subject}"
