...
from app.db.models.workflow_run import NodeAttemptStatus
...
class WorkflowReconcilerService:
    ...
    async def resolve_orphaned_attempts(self, *, limit: int = 100) -> int:
        ...
        for attempt in attempts:
            node_run = await workflow_run_repo.get_node_run_by_id(self.db, attempt.node_run_id)
            if node_run is None:
                continue
            run = await workflow_run_repo.get_run_by_id_for_update(
                self.db, node_run.workflow_run_id
            )
            # Re-read the attempt under lock: `begin_attempt`'s own inline
            # check may have resolved it between this sweep's scan and now.
            fresh = await workflow_run_repo.get_attempt(self.db, attempt.id)
            if run is None or fresh is None or fresh.status != NodeAttemptStatus.IN_FLIGHT.value:
                continue
            outbox = await workflow_run_repo.get_outbox_for_node_run(
                self.db, node_run_id=node_run.id
            )
