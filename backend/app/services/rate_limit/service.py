...
def make_rate_limit_dep(category: str):
    ...
    async def _dep(request: Request, user: CurrentUser, active_org: ActiveOrg) -> None:
        ...
        try:
            ...
            # Plan features are an optimisation; the default limits apply if the
            # subscription cannot be read. Logged because a persistent failure
            # here silently downgrades every paying customer.
            logger.warning("rate_limit_plan_features_unavailable", exc_info=True)
