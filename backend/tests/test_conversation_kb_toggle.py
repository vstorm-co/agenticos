...
def _conv(active_kb_ids=None, organization_id=None):
    conv = MagicMock()
    conv.id = "conv-1"
    conv.active_knowledge_base_ids = active_kb_ids
    conv.user_id = "user-1"
    conv.organization_id = organization_id or TEST_ORG_ID
    return conv
...
class TestConversationKBToggle:
    ...
    async def test_update_sets_explicit_kb_list(self, mock_db):
        ...
        with (
            ...
            result = await svc.update_kb_settings(
                "conv-1", ["kb-1", "kb-2"], user_id="user-1", organization_id=TEST_ORG_ID
            )
    ...
    async def test_update_to_none_restores_defaults(self, mock_db):
        ...
        with (
            ...
            svc = ConversationService(mock_db)
            result = await svc.update_kb_settings(
                "conv-1", None, user_id="user-1", organization_id=TEST_ORG_ID
            )
    ...
    async def test_update_to_empty_disables_rag(self, mock_db):
        ...
        with (
            ...
            svc = ConversationService(mock_db)
            result = await svc.update_kb_settings(
                "conv-1", [], user_id="user-1", organization_id=TEST_ORG_ID
            )
    ...
    async def test_update_kb_settings_passes_correct_update_data(self, mock_db):
        ...
        with (
            ...
            await svc.update_kb_settings(
                "conv-1", ["kb-x"], user_id="user-1", organization_id=TEST_ORG_ID
            )
    ...
    async def test_update_kb_settings_other_user_raises(self, mock_db):
        ...
        with (
            ...
            with pytest.raises(NotFoundError):
                await svc.update_kb_settings(
                    "conv-1", ["kb-1"], user_id="other", organization_id=TEST_ORG_ID
                )
