...
- **Chat attachments reach the model again.** A chat turn linked its uploaded
  ...
  with no reply. The run now reads its own turn's files by the message they were
  just linked to, so `list_attached_files`' unlinked-guard still protects a fresh
  submission without rejecting the turn's own files. (#1756)
