...
```bash
uv run agenticos cmd rag-ingest ./docs/ --collection docs --recursive   # ingest files/folder
uv run agenticos cmd rag-search "your question" --collection docs        # semantic search
uv run agenticos cmd rag-collections                                     # list collections
uv run agenticos cmd rag-stats                                           # chunk/vector counts
uv run agenticos cmd rag-drop <collection> --yes                         # delete a collection
```
...
```bash
uv run agenticos cmd rag-sources                  # list
uv run agenticos cmd rag-source-add               # add (interactive)
uv run agenticos cmd rag-source-sync --all        # trigger a sync
```
