...
4. **Event Subscriptions → Enable Events → Subscribe to bot events**, and add the
   five events below.
5. Register the bot: **Channels → Add channel**, platform `slack`, paste
   the bot token.
...
2. **Channels → Add channel**, platform `telegram`.
...
2. Register it: **Channels → Add channel**, platform `mattermost`, paste
...
is the source language; `pl.json` and `de.json` hold only what has actually been
translated - `src/i18n.ts` merges English underneath every locale, so a missing
translation renders English rather than the key. `make lint` runs `frontend/scripts/check-i18n.ts`, which
