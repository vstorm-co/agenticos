...
| `chat_files` | Cascade; the bytes are unlinked after the commit | Not listed | The row cascades from the message and the file on disk does not, so the paths are collected before the delete and unlinked after it commits - otherwise the bytes outlive the request that asked for them to go, reachable by nothing and deleted by nobody. The transcript in the export names the attachment; the bytes are downloadable while the account exists, not packaged into it |
...
| A changed email address is proved before mail follows it | `PATCH /users/me` stages the address in `users.pending_email` and mails a single-use, one-hour link to it; the account keeps receiving everything at its current address until that link comes back, and the current address is told a change was asked for. The link carries the account's credential version, so changing or resetting the password — what that notice tells its owner to do — revokes it, and an administrator repairing the address clears the staging. Asking again for the address already staged sends nothing, and distinct addresses are capped per account per hour. Both the request and the confirmation are audited (`app/services/user.py`, `POST /auth/email-change/confirm`) | `test_email_change.py` |
...
| Signing in with a directory account | The standard two-step LDAP bind - search as the service account, then bind **as the account** with the typed password; an empty password never reaches a bind, the username is escaped into the filter, two matching accounts are a refusal, TLS is verified and `ldap://` without StartTLS is refused at startup (`app/services/directory/ldap_directory.py`). See [Directory sign-in](directory.md#signing-in-with-a-directory-account) | `test_ldap_directory.py`, `test_directory_settings.py` |
| Integrated Windows sign-in | SPNEGO with this service's keytab; only a ticket accepted in one step is accepted, and its principal is resolved through the directory to the same account the password reaches (`app/services/directory/kerberos.py`, `app/api/routes/v1/directory_auth.py`) | `test_kerberos_acceptor.py`, `test_directory_auth_routes.py` |
| Directory groups decide memberships | Directory group mappings, applied at every LDAP, Kerberos or OIDC sign-in. The sync rewrites only memberships it made, never demotes or removes an owner, never joins a personal organization, and cannot map a role its author could not assign; an Entra ID group overage is refused (`app/services/directory/sync.py`, `app/services/directory/mappings.py`, `groups_claim` in `app/core/oauth.py`) | `tests/integration/test_directory_groups.py`, `test_oidc_groups.py` |
| Grants to groups | A grant has exactly one subject, a person or a group (`ck_resource_grant_one_subject`); a group grant reaches whoever is in the group when access is checked, and only through groups of the same organization (`app/repositories/resource_grant.py`) | `tests/integration/test_directory_groups.py`, `test_resource_grant_repo.py` |
| SAML, SCIM, a directory re-check between sign-ins | **Not yet** — a disabled directory account's open session lasts until its refresh token expires, unless an administrator deactivates it. See [What this does not do yet](directory.md#what-this-does-not-do-yet) | — |
...
| Tamper evidence (a hash chain) | Every entry joins a per-organization hash chain, and each chain carries a checkpoint at its high-water mark, so a rewritten entry, a dropped tail and a deleted chain are each detectable; `agenticos cmd audit-verify` walks them and exits non-zero on a break (`app/core/audit.py`, `app/commands/audit_verify.py`). Detection, not prevention: whoever holds the database's own credentials can re-forge a chain or drop the checkpoint's trigger | `test_audit_record.py`, `test_audit_chain_backfill.py`, `test_audit_verify_command.py`, `test_audit_checkpoint_migration.py` |
...
| `sso` | §164.312(d) | `OIDC_ISSUER`, configured by discovery alone - the issuer is the only URL, and the authorization, token and JWKS endpoints come from its `.well-known/openid-configuration` - or `KERBEROS_ENABLED`, sign-in with the domain's own ticket. Neither set, the control is unmet and the sheet says so: with `LDAP_URL` alone it says the passwords are the directory's but typed here with no single sign-on, and otherwise that people sign in with passwords this deployment stores. Multi-factor authentication is the identity provider's, and the sheet does not claim it either way |
