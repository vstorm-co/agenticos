...
| A changed email address is proved before mail follows it | `PATCH /users/me` stages the address in `users.pending_email` and mails a single-use, one-hour link to it; the account keeps receiving everything at its current address until that link comes back, and the current address is told a change was asked for. Both the request and the confirmation are audited (`app/services/user.py`, `POST /auth/email-change/confirm`) | `test_email_change.py` |
