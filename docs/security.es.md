...
| `chat_files` | Cascada; los bytes se desvinculan tras el commit | No se enumera | La fila cascadea desde el mensaje y el fichero en disco no, así que las rutas se recogen antes del borrado y se desvinculan tras su commit; de otro modo los bytes sobreviven a la petición que pedía quitarlos: inalcanzables para nada y borrados por nadie. La transcripción del export nombra el adjunto; los bytes se descargan mientras la cuenta exista, no se empaquetan dentro |
...
| Inicio de sesión con una cuenta del directorio | El bind LDAP estándar en dos pasos - buscar como la cuenta de servicio y luego hacer bind **como la cuenta** con la contraseña tecleada; una contraseña vacía nunca llega a un bind, el nombre de usuario se escapa al insertarlo en el filtro, dos cuentas coincidentes son un rechazo, TLS se verifica y `ldap://` sin StartTLS se rechaza al arrancar (`app/services/directory/ldap_directory.py`). Véase [Inicio de sesión con el directorio](directory.md#signing-in-with-a-directory-account) | `test_ldap_directory.py`, `test_directory_settings.py` |
| Inicio de sesión integrado de Windows | SPNEGO con el keytab de este servicio; solo se acepta un ticket aceptado en un único paso, y su principal se resuelve a través del directorio a la misma cuenta a la que llega la contraseña (`app/services/directory/kerberos.py`, `app/api/routes/v1/directory_auth.py`) | `test_kerberos_acceptor.py`, `test_directory_auth_routes.py` |
| Los grupos del directorio deciden las membresías | Mapeos de grupos del directorio, aplicados en cada inicio de sesión por LDAP, Kerberos u OIDC. La sincronización reescribe solo las membresías que ella misma creó, nunca degrada ni elimina a un propietario (owner), nunca incorpora a nadie a una organización personal y no puede mapear un rol que su autor no podría asignar; un exceso de grupos (*group overage*) de Entra ID se rechaza (`app/services/directory/sync.py`, `app/services/directory/mappings.py`, `groups_claim` en `app/core/oauth.py`) | `tests/integration/test_directory_groups.py`, `test_oidc_groups.py` |
| Concesiones a grupos | Una concesión tiene exactamente un sujeto, una persona o un grupo (`ck_resource_grant_one_subject`); una concesión a un grupo alcanza a quien esté en el grupo cuando se comprueba el acceso, y solo a través de grupos de la misma organización (`app/repositories/resource_grant.py`) | `tests/integration/test_directory_groups.py`, `test_resource_grant_repo.py` |
| SAML, SCIM, una nueva comprobación del directorio entre inicios de sesión | **Todavía no** — la sesión abierta de una cuenta desactivada en el directorio dura hasta que caduca su refresh token, salvo que un administrador la desactive. Véase [Lo que esto todavía no hace](directory.md#what-this-does-not-do-yet) | — |
...
| Evidencia de manipulación (una cadena de hashes) | Cada entrada se suma a la cadena de hashes de su organización, y cada cadena lleva un checkpoint en su marca más alta, así que una entrada reescrita, una cola truncada y una cadena borrada se detectan por separado; `agenticos cmd audit-verify` las recorre y sale con código distinto de cero ante una rotura (`app/core/audit.py`, `app/commands/audit_verify.py`). Detección, no prevención: quien tenga las credenciales de la propia base de datos puede reforjar una cadena o quitar el trigger del checkpoint | `test_audit_record.py`, `test_audit_chain_backfill.py`, `test_audit_verify_command.py`, `test_audit_checkpoint_migration.py` |
...
| Las páginas escritas por agents no pueden alcanzar la consola | Un [artefacto](artifacts.md) publicado se sirve desde una ruta sin cookies tras un token firmado de vida corta, bajo `Content-Security-Policy: sandbox` sin `allow-same-origin` — un origen opaco — ni `allow-popups`, más `connect-src 'none'` y `frame-ancestors` nombrando solo la consola; opcionalmente desde su propio dominio registrable (`ARTIFACT_ORIGIN`) | `test_artifact_routes.py::TestTheContentRoute`, `test_artifact_service.py::TestThePolicy`, `artifacts.test.tsx`, `csp.test.ts` |
| Límites de peticiones en las superficies públicas | Límites sobre Redis en la API de runs, el widget embed, las páginas alojadas, los enlaces públicos de artefactos y sus direcciones de contenido firmadas (`app/services/rate_limit.py`); límites por remitente en los bots de canal (`app/services/channels/router.py`) | `test_rate_limited_surfaces.py`; el límite del bot de canal está implementado, pero poco testeado |
...
| `sso` | §164.312(d) | `OIDC_ISSUER`, configurado solo por discovery: el issuer es la única URL, y los endpoints de autorización, token y JWKS salen de su `.well-known/openid-configuration` - o `KERBEROS_ENABLED`, el inicio de sesión con el propio ticket del dominio. Sin ninguno de los dos, el control está incumplido y la hoja lo dice: con `LDAP_URL` solo, dice que las contraseñas son las del directorio pero se teclean aquí sin inicio de sesión único, y si no, que la gente entra con contraseñas que este despliegue guarda. La autenticación multifactor es del proveedor de identidad, y la hoja no se la atribuye en ningún sentido |
