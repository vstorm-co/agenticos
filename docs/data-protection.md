...
| `virtual_table_records`, `virtual_table_record_history`, `virtual_table_receipts`, `virtual_table_outbox` | The rows of a [Virtual Table](virtual-tables.md) and the trail of every change to them | Whatever the organization put in its cells, which can be personal data. The current values (records); the values before and after every change (history); the whole record as a keyed write returned it (receipts); the ids and external id of each created record (outbox, no values). A record delete removes only the current row: history and receipts keep the values, and none of the three trail tables has a retention class or a purge yet. Deleting an account removes its receipts and clears it as the actor on history; deleting the organization removes all four | Typed records for agents and workflows, safe retries of a write, and the hand-off of a created record |
...
| `workflow_runs`, `node_runs`, `node_attempts`, `workflow_events`, `dispatch_outbox`, `resource_refs` | Each run of a workflow, every node's attempts and the run's event stream | The account the run acts as; each attempt's result, which holds whatever the node produced from the data it was given; a failure's code and message; the file and table references a graph bound. The rows go with their workflow and organization, the account is cleared from a run when it is deleted, and none of the tables has a retention class or a purge yet | Running a workflow to completion across restarts, and showing its history |
