...
| The browsing decision model | Every step of a browse: the page's URL, its title and the labels of the elements in view - which may be the contents of an internal system | The `browser_choice` capability, which cannot run without a vault key an operator adds; `decision_base_url` moves the destination | The decision model vendor's, or your own host |
