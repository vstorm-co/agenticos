...
| `notifications`, `notification_deliveries` | Every in-app row and its email delivery | The recipient, a pre-rendered summary (never a raw comment or secret value) and the typed variables an email re-renders from; a delivery row adds only its channel, status and attempts | Alerts a person reads once, unattended. See [Governance](governance.md#alerts) |
