...
| Trazas | `observability.content` por agent: `full` registra todo, `none` solo tiempo, tokens, coste y nombres de herramienta | [Entornos](environments.md) (#1413). Dos caminos aún no respetan un `none`: un especialista inline de ese agent ([#1699](https://github.com/vstorm-co/agenticos/issues/1699)) y un run ejecutado por Prefect, al que el worker no instrumenta en absoluto ([#1700](https://github.com/vstorm-co/agenticos/issues/1700)) |
...
nombres de herramienta, y ningún texto de mensaje sale.

Entre ambos no hay nada, deliberadamente. Un tercer modo que exportara el
contenido pasado por un filtro de PII se
[consideró y se descartó](https://github.com/vstorm-co/agenticos/issues/1616):
un filtro que se deje un campo es una garantía que nadie puede auditar, y las
trazas valen mucho menos para la depuración para la que existen si se les
recorta el contenido. Un deployment que no pueda exportar texto de mensajes pone
`none`, o no pone ningún token.

`none` se sostiene tanto en el tracer por agent como en el de nivel de
deployment, con dos huecos que una revisión debería conocer: un especialista
inline de ese agent no lleva bloque de observability propio y el del padre no lo
fija ([#1699](https://github.com/vstorm-co/agenticos/issues/1699)), y el worker
de Prefect no configura Logfire en absoluto, así que un run programado falta en
las trazas en lugar de aparecer recortado
...
- El `content: none` de un agent no alcanza a sus especialistas inline
  ([#1699](https://github.com/vstorm-co/agenticos/issues/1699)), y un run que
  ejecuta el worker de Prefect no se traza en absoluto
  ([#1700](https://github.com/vstorm-co/agenticos/issues/1700)).
...
(#1413) y la matriz de controles de HIPAA y SOC 2 en
