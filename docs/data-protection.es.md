...
| El modelo de decisión de la navegación | Cada paso de un recorrido: la URL de la página, su título, las etiquetas de los elementos a la vista y hasta 1.500 caracteres de su texto visible, que pueden ser el contenido de un sistema interno. Un valor que el agent escribe **no** se envía: el paso se registra como «rellenado» sin él, así que una contraseña o una dirección no llegan a ese endpoint | La capability `browser_choice`, que no se ejecuta sin una clave del vault que añada un operador; `decision_base_url` cambia el destino. Qué navegador dirige lo limita `BROWSER_CDP_ALLOWED_HOSTS`, vacío por defecto | El proveedor del modelo de decisión, o tu propio host |
