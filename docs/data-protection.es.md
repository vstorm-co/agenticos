...
| `workflow_runs`, `node_runs`, `node_attempts`, `workflow_events`, `dispatch_outbox`, `resource_refs` | Cada run de un workflow, los intentos de cada nodo y el flujo de eventos del run | La cuenta en cuyo nombre actúa el run; el resultado de cada intento, que contiene lo que el nodo produjo con los datos que recibió; el código y el mensaje de un fallo; las referencias a archivos y tablas que vinculó un grafo. Las filas se van con su workflow y su organización, la cuenta se borra del run cuando se elimina, y ninguna de las tablas tiene aún clase de retención ni purga | Llevar un workflow hasta el final a través de reinicios, y mostrar su historial |
