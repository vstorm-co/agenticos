...
| Búsqueda, navegación, memoria, herramientas | No vincules ningún secreto `search`, ninguna capability `web_fetch`, `browser_use` ni `memory_mem0`, ninguna conexión MCP. `browser_choice` puede quedarse, con `decision_base_url` apuntando a un modelo de decisión que alojes tú |
...
| `virtual_table_records`, `virtual_table_record_history`, `virtual_table_receipts`, `virtual_table_outbox` | Las filas de una [Virtual Table](virtual-tables.md) y el rastro de cada cambio en ellas | Lo que la organización puso en sus celdas, que puede ser dato personal. Los valores actuales (records); el registro completo en un create y un delete, y solo las celdas que cambiaron en un update (history); el registro completo tal como lo devolvió una escritura con clave (receipts); los ids y el external id de cada registro creado (outbox, sin valores). Borrar un registro elimina solo la fila actual: history y receipts conservan los valores hasta que el barrido diario de retención los elimina: los receipts a las 24 horas, las filas de outbox despachadas a los 3 días y el history a los 365 días, todo como ajustes del deployment. Borrar una cuenta elimina sus receipts y la borra como actor en history; borrar la organización elimina las cuatro | Registros tipados para agents y workflows, reintentos seguros de una escritura y el traspaso de un registro creado |
...
| Tu relay SMTP | Invitaciones, enlaces mágicos, peticiones de aprobación, alertas de budget, informes de uso, y el propio correo del [buzón de notificaciones](governance.md#alerts) - un run que termina o falla desatendido, la ingesta de un documento que se completa o falla, un evento de seguridad o un cambio de configuración, el propio anuncio de un admin | Lo que el relay conserve. El correo de aprobación nombra el agent, la herramienta y un enlace, no los argumentos de la herramienta |
...
| El modelo de decisión de la navegación | Cada paso de un recorrido: la URL de la página, su título, las etiquetas de los elementos a la vista y hasta 1.500 caracteres de su texto visible, que pueden ser el contenido de un sistema interno. Un valor que el agent escribe **no** se envía, ni tampoco lo que un campo ya contiene: la tabla de elementos solo indica que un campo está rellenado, así que una contraseña o una dirección no llegan a ese endpoint | La capability `browser_choice`, que no se ejecuta sin una clave del vault que añada un operador; `decision_base_url` cambia el destino. Qué navegador dirige lo limita `BROWSER_CDP_ALLOWED_HOSTS`, vacío por defecto | El proveedor del modelo de decisión, o tu propio host |
...
| Identidad corporativa | Inicio de sesión con Google, contraseñas, OIDC genérico contra el proveedor que ya tienes — configurado solo por discovery desde `OIDC_ISSUER` — y tu propio directorio por LDAP o Kerberos. La dirección, el nombre visible y el identificador estable del directorio se guardan en la cuenta; sus grupos se leen en cada inicio de sesión y no se guardan, solo las membresías que los mapeos de las organizaciones hacen de ellos. Sin SAML ni SCIM | [Inicio de sesión con el directorio y grupos](directory.md); `OIDC_ISSUER`, `LDAP_URL` |
