...
| `workflow_runs`, `node_runs`, `node_attempts`, `workflow_events`, `dispatch_outbox`, `resource_refs` | Jeder Run eines Workflows, die Versuche jedes Knotens und der Ereignisstrom des Runs | Das Konto, in dessen Namen der Run handelt; das Ergebnis jedes Versuchs, das enthält, was der Knoten aus den ihm übergebenen Daten erzeugt hat; Code und Meldung eines Fehlers; die Datei- und Tabellenverweise, die ein Graph gebunden hat. Die Zeilen verschwinden mit ihrem Workflow und ihrer Organisation, das Konto wird beim Löschen aus dem Run entfernt, und keine der Tabellen hat bisher eine Aufbewahrungsklasse oder eine Bereinigung | Einen Workflow über Neustarts hinweg zu Ende führen und seine Historie zeigen |
