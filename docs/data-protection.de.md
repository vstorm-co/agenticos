...
| `virtual_table_records`, `virtual_table_record_history`, `virtual_table_receipts`, `virtual_table_outbox` | Die Zeilen einer [Virtual Table](virtual-tables.md) und die Spur jeder Änderung daran | Was die Organisation in ihre Zellen geschrieben hat, darunter möglicherweise personenbezogene Daten. Die aktuellen Werte (Records); der ganze Datensatz bei Create und Delete, bei einem Update nur die geänderten Zellen (History); der ganze Datensatz, wie ihn ein Schreibzugriff mit Schlüssel zurückgab (Receipts); die ids und die external id jedes erstellten Datensatzes (Outbox, keine Werte). Ein Löschen des Datensatzes entfernt nur die aktuelle Zeile: History und Receipts behalten die Werte, bis der tägliche Aufbewahrungs-Sweep sie entfernt: Receipts nach 24 Stunden, zugestellte Outbox-Zeilen nach 3 Tagen und die History nach 365 Tagen, alles Einstellungen des Deployments. Das Löschen eines Kontos entfernt seine Receipts und löscht es als Akteur in der History; das Löschen der Organisation entfernt alle vier | Typisierte Datensätze für Agents und Workflows, sichere Wiederholungen eines Schreibzugriffs und die Übergabe eines erstellten Datensatzes |
...
| `notifications`, `notification_deliveries` | Jede an jemanden adressierte Benachrichtigung und ihre E-Mail-Zustellung. Eine Zeile wird auch dann geschrieben, wenn der Empfänger für dieses Ereignis jeden Kanal abgeschaltet hat - sie ist es, die verhindert, dass dasselbe Vorkommnis zweimal geschrieben wird - und wird dann einfach nie gezeigt oder gesendet; nur die Zustellungszeile ist bedingt | Der Empfänger, eine vorgerenderte Zusammenfassung (nie ein roher Kommentar oder Secret-Wert) und die typisierten Variablen, aus denen eine E-Mail neu rendert; eine Zustellungszeile fügt ihren Kanal, Status, die Versuche und einen bereinigten Fehlgrund hinzu — nie den Fehlertext des Providers | Warnungen, die eine Person einmal liest, unbeaufsichtigt. Siehe [Governance](governance.md#alerts) |
| `notification_preferences`, `announcements` | Kanal-Umschalter je Ereignis, und die eigenen Ankündigungen eines App-Administrators | Eine Benutzer-Id je Umschalter; der Absender einer Ankündigung, ihr Text sowie die Organisationen und die Rolle, an die sie adressiert war — nie die aufgelöste Empfängerliste, die aus `notifications` nur rekonstruierbar ist, solange diese Zeile nicht durch [den Retention-Sweep für Benachrichtigungen](governance.md#alerts) verfällt — danach hält der Audit-Eintrag dahinter nur noch eine Zahl, nie die Empfänger | Opt-out, und der eigene Nachweis des Verfassers |
...
| Ihr SMTP-Relay | Einladungen, Magic Links, Freigabeanfragen, Budget-Warnungen, Nutzungsberichte, und die eigene Mail des [Benachrichtigungs-Postfachs](governance.md#alerts) - ein Run, der unbeaufsichtigt fertig wird oder scheitert, die Ingestion eines Dokuments, die abschließt oder scheitert, ein Sicherheitsereignis oder eine Konfigurationsänderung, die eigene Ankündigung eines Admins | Was das Relay aufbewahrt. Eine Freigabe-Mail nennt den Agent, das Tool und einen Link, nicht die Argumente des Tools |
...
| Unternehmensidentität | Google-Anmeldung, Passwörter, generisches OIDC gegen den Provider, den Sie ohnehin betreiben - allein per Discovery aus `OIDC_ISSUER` konfiguriert - und Ihr eigenes Verzeichnis über LDAP oder Kerberos. Adresse, Anzeigename und stabile Kennung aus dem Verzeichnis werden am Konto gehalten; seine Gruppen werden bei jeder Anmeldung gelesen und nicht gespeichert, nur die Mitgliedschaften, die die Zuordnungen der Organisationen daraus machen. Kein SAML, kein SCIM | [Verzeichnisanmeldung und Gruppen](directory.md); `OIDC_ISSUER`, `LDAP_URL` |
...
Wiederholbare Prüfungen, vom Host aus, gegen das laufende Deployment. Jede gibt
Fakten aus, die die Prüfung anhängen kann; keine gibt ein Zugangsdatum oder die
Daten einer Person aus. Führe sie aus `backend/` aus, oder über
`docker compose exec api`.

```bash
# 1. Läuft es überhaupt, und sind die Verbindungen zu den Speichern
#    verschlüsselt? `postgres` meldet den TLS-Zustand der Verbindung, die der
#    Doctor selbst aufgebaut hat.
uv run agenticos cmd doctor

# 2. Jedes versiegelte Zugangsdatum öffnet sich weiterhin unter den
#    konfigurierten Master Keys.
uv run agenticos cmd vault-rotate --dry-run

# 3. Die Hash-Ketten der Audit-Spur und ihre Checkpoints, neu berechnet. Endet
#    mit einem Exit-Code ungleich null, wenn eine Kette verändert wurde.
uv run agenticos cmd audit-verify

# 4. Alles, was dieses Deployment tatsächlich konfiguriert hat: die
...
Punkt 4 ist der, den man anhängt. Er gibt Konfiguration und Zählungen aus und
niemals Inhalt: keinen Nachrichtentext, kein Dokument, keinen Secret-Wert und
keinen Hinweis darauf, und eine Einstellung, die ein Zugangsdatum hält, wird als
gesetzt oder ungesetzt gemeldet statt ausgegeben. `--older-than` ist die
...
**Im Deployment, von seinem Betreiber entschieden:** die Vereinbarungen,
Standorte, Trainingsausschlüsse, der Aufbewahrungsplan, die Backup-Frist, die
Plattenverschlüsselung, der Sandbox-Ausgangsverkehr und die Rechtsseiten des
vorherigen Abschnitts. Jedes davon ist der Nachweis, den
`data-protection-report` bewusst nicht erzeugt.
