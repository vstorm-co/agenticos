...
1. Okno widzi nawigację do `/api/oauth/<dostawca>/login`, przekazuje ją
   przeglądarce systemowej z dopisanym `client=desktop` i za nią nie idzie. Nic w
   konsoli nie wie, że działa w shellu, i nie musi.
2. Logujesz się tam, w prawdziwej przeglądarce, ze swoimi hasłami, swoimi
   rozszerzeniami i jakiegokolwiek drugiego składnika używasz.
3. Callback wdrożenia widzi `client=desktop` — zapisane w sesji na *starcie*,
   nigdy czytane z powrotu — i przekierowuje na
   `agenticos://auth/callback?code=…` z jednorazowym kodem, który wygasa po
   minucie.
4. System operacyjny podaje ten link aplikacji. Shell wysyła okno konsoli na
