...
| Model decyzyjny przeglądania | Każdy krok przeglądania: adres strony, jej tytuł i etykiety widocznych elementów — co może być zawartością systemu wewnętrznego | Capability `browser_choice`, która nie zadziała bez klucza z vault dodanego przez operatora; `decision_base_url` zmienia miejsce docelowe | Dostawcy modelu decyzyjnego albo twojego własnego hosta |
