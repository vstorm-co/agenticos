...
**The operating system for your company's AI agents.** Self-hosted, open source,
and yours: agents are configuration you own, running on your infrastructure,
against your keys.

An agent here is *data*, not code — instructions, a model, a set of capabilities.
You build it in a UI, publish a version, and it runs the same way everywhere:
web chat, HTTP API, Slack. Budgets, approvals and audit apply identically to all
of them, because every surface goes through one runner.
...
Leave `BOOTSTRAP_API_KEY` out and everything is still created — the agent just
cannot run until you add a key under **Settings → AI providers**. That is a
deliberate half-built state rather than a silent failure.

`make platform-bootstrap` is idempotent. Re-run it whenever you are not sure it
worked.
...
| **Capabilities** | Knowledge search, web research, charts, code execution — switched on per agent, never edited as code in a browser |
...
| **Governance** | Per-run and monthly budgets that stop a run, human approval for anything side-effecting, an audit trail |
| **Access** | Permission catalog in code, roles composed from it, per-resource sharing |
| **Multi-tenant** | Organization isolation enforced in the schema, not only in the code |

Secrets are sealed per organization: a key copied from one tenant's database row
cannot be decrypted for another, and no API response ever returns one.
...
│   └── tasks/            # Distributed tasks (prefect)
└── commands/             # Click CLI commands (auto-discovered by `agenticos cmd …`)
...
The generated project ships a Click CLI exposed as `agenticos` (after `make install`):

```bash
agenticos server run --reload          # dev server
agenticos db upgrade                   # apply migrations
agenticos db migrate -m "message"      # create new migration
agenticos user create-admin            # interactive admin creation
agenticos rag-ingest <path> -c docs    # ingest local files
agenticos rag-search "query" -c docs   # semantic search
agenticos rag-collections              # list collections
```
...
Run `make help` for a categorized list, or `agenticos --help` for full CLI docs.
...
agenticos rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
agenticos rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
agenticos rag-sync-s3 --collection documents --prefix docs/

# Semantic search
agenticos rag-search "your query" --collection documents
```
...
| Guide | What |
|-------|-------|
| `docs/channels.md` | Put an agent on a website, in Slack, Telegram or Mattermost |
| `docs/howto/add-api-endpoint.md` | Add a new REST endpoint |
| `docs/howto/add-capability.md` | Add a capability — the unit an agent is assembled from |
...
Apache-2.0 rather than MIT because AgenticOS is meant to be deployed inside
other companies: the explicit patent grant is the part their legal review asks
about, and MIT is silent on it.
